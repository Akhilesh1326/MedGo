...
const Patient_PI = () => {
  ...
  function validatInput(maritalStatus,occupation,languagePreference,emergencyContactNumber,emergencyContactName) {

    if(maritalStatus== "" && occupation == "" && languagePreference == "" && emergencyContactNumber == "" && emergencyContactName == ""){
      setError("Fill all the fields before continuing")
      return false
    }
    else if (maritalStatus == "") {
      setError("Fill the marital status Field before continuing !")
      return false
    }
    else if (occupation == ""){
      setError("Fill the occupation  Field before continuing !")
      return false
    }
    else if (languagePreference == ""){
      setError("Fill the language preference Field before continuing !")
      return false
    }
    else if (emergencyContactNumber == ""){
      setError("Fill the emergency contact number Field before continuing !")
      return false
    }
    else if (emergencyContactName == ""){
      setError("Fill the emergency contact name Field before continuing !")
      return false
    }
    ...
    if(!phoneRegex.test(Number(emergencyContactNumber))){
      setError("Please enter the appropriate size of phone digits ECN")
      return false
    }
  ...
  const handlePIFormSubmit = async () => {
    if(!validatInput(maritalStatus,occupation,languagePreference,emergencyContactNumber,emergencyContactName)){
      return
    }
  ...
  return (
    <div>
      <div className="flex flex-col justify-center items-center z-0 h-screen"
        ...
        <h1 className='font-bold text-2xl text-[#327dbe] mt-20'>Personal Information</h1>
        <div className='font-bold text-xl text-red-600 mt-10'>{error}</div>

        <div className="flex lg:border-[#4eacff] lg:border-4 rounded-xl mb-20 md:w-[80%]">
          ...
          <div className="flex flex-col w-screen lg:bg-[#0d4576] opacity-90 lg:rounded- rounded-bl-3xl rounded-tl-3xl rounded-r-lg ">
            ...
            <select className=" lg:w-[70%] lg:text-white lg:self-center lg:m-0 ml-4 sm:ml-8 md:ml-12 rounded-lg py-1 px-1 lg:border-[#35b5dc] lg:border-b-2 lg:border-0 border-2 lg:outline-none lg:rounded-lg lg:hover:shadow-[#38a2ff] lg:hover:shadow-[0px_10px_10px_0px] lg:focus:hover:shadow-[#0688b0] lg:focus:hover:shadow-[0px_10px_10px_0px]  lg:focus-within:pl-5 shadow-lg font-semibold w-[90%] h-12 lg:bg-[#0d4576] focus-within:-translate-y-1 hover:-translate-y-1 duration-300 opacity-100 " value={maritalStatus} onChange={(e) => setMaritalStatus(e.target.value)}>
              <option className='lg:text-white w-10' value="Select Gender">Select Status</option>
              <option className='lg:text-white w-10' value="male">Not Married</option>
              <option className='lg:text-white w-10' value="male">Married</option>
