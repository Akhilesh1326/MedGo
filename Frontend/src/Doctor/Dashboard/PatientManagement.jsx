            ...
            <div className="grid grid-cols-4 gap-4 mx-10 place-items-center">
                ...
                {OfflinePatientData.map((item) => (
                    <div key={item._id} className="flex flex-col border-2 border-black rounded-lg w-fit items-center py-0 px-4">
                        ...
                        <div>{item.dob}</div>
                        <button className="bg-slate-800 text-white border-2 border-black rounded-full" onClick={() => toggleOfflinePatientDetails(item._id)}>
                            Check Patient
                        </button>
                        {selectedOfflinePatient === item._id && (
                            <div className="bg-slate-700 absolute z-40 flex flex-col border-2 border-black rounded-lg ml-[400px] w-[450px]">
                                ...
                                {/* More details */}
                                <button className="bg-slate-800 text-white border-2 border-black rounded-full w-fit" onClick={() => toggleOfflinePatientDetails(item._id)}>
                                    Exit Details
                                </button>
                            </div>
                        )}
                ...
                {onlinePatientData.map((item) => (
                    <div key={item.appointmentId} className="flex flex-col border-2 border-black rounded-lg w-fit items-center py-0 px-4">
                        <div className="text-sm">{item.patientType}</div>
                        <img src="" alt="img" className="h-16 w-16 border-2 border-black rounded-full mt-2" />
                        <div>{item.fullName}</div>
                        <div>{item.contactNumber}</div>
                        <div>{item.dob}</div>
                        <button className="bg-slate-800 text-white border-2 border-black rounded-full" onClick={() => toggleOnlinePatientDetails(item.appointmentId)}>
                            Check Patient
                        </button>
                        {selectedOnlinePatient === item.appointmentId && (
                            <>
                                ...
                                <div className="bg-slate-700 absolute z-40 flex flex-col border-2 border-black rounded-lg ml-[400px] w-[450px]">
                                    ...
                                    <button className="bg-slate-800 text-white border-2 border-black rounded-full w-fit py-2 px-4 my-1 mx-2" onClick={() => openNewPrescriptionForm(item)}>
                                        Add Prescription
                                    </button>
                                    <button className="bg-slate-800 text-white border-2 border-black rounded-full w-fit py-2 px-4 my-1 mx-2" onClick={() => { setOpenViewPrescription(true), handleGetAllPrescription(item.appointmentId) }}>
                                        View Prescription
                                    </button>
                                    </div>
                                </div>
